...
///the code overall is mostly readable, see also additional remarks below
...
bool possible_sequence(std::string digits, std::string sequence){
  ...
  ///You should use the function string_to_digits
  ///See assignment instructions: https://intranet.ee.ic.ac.uk/m.cattafi/scripts/dw/doku.php?id=progeng2223:a1
...
/// in order to emphasise invalid indexing
...
int get_element(const std::vector<int>& v, int i){
    if(i < v.size()){
        ...
        std::cout << "invalid indexing: attempting access with index " << i << " for vector of size " << v.size() << std::endl;
...
int score(std::string s, const std::vector<int>& values){
  ...
  for(int i=0; i<ints.size(); i++){
    ...
    // replacing
        ...
        /// with the following line for debugging in order to emphasise the invalid indexing causing undefined behaviour

    if(ints[i]+c == get_element(ints, i+c) && next<=ints.size()){
      if(ints.size()==2){
...
int get_best_sequence(std::string digits, const std::vector<std::string>& allowed, const std::vector<int>& values, std::string& best_sequence){
    int highestscore = -2147483545;///Use INT_MIN
...
int main(){
    ...
    /// there is no need for a different ifstream for each file, you can reuse the same one after you close it
